...
<title><%RESPONSE.WRITE(SEMESTER & " ICE10")%></title>
</head>
<body>
<% 
...
const INSTRUCTOR = "Paulson" 
  ...
  Term=cstr(request.form("lstSemester"))
   ...
   cSQL = " SELECT tblHW.RecordID as ID, tblHW.TimeTrack, tblhw.q1, tblhw.q2, tblhw.q3, tblhw.q4 FROM tblHW WHERE tblHW.InstID= '00617282' AND tblHW.Semester= 'Spring2016' AND tblHW.Assignment= 'ICE10' AND tblHW.Class= 'MIS362' ORDER BY tblHW.LastName, tblhw.timetrack DESC " 
...
objConnSQL.open  "Driver={SQL Server};" & _
 ...
 <% response.write("    ICE10 Responses" & Assignment ) %> </H2>
